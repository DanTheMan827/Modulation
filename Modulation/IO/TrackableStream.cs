/*
    TrackableStream.cs
    Copyright (C) 2018  DanTheMan827
    Copyright (C) 2018  daPhie79
...
namespace DanTheMan827.Modulation.IO
{
    public class TrackableStream : MemoryStream
    {
        ...
        {
            if (this.internalStream is not null)
            {
                this.internalStream.Write(array, offset, count);
                OnProgress(this.internalStream.Position, this.internalStream.Length);
            }
            else
            {
        ...
        public override int Read(byte[] array, int offset, int count)
        {
            ...
            if (this.internalStream is not null)
            {
                r = this.internalStream.Read(array, offset, count);
                this.internalStreamProgress += r;
                OnProgress(this.internalStreamProgress, this.Length);
            }
            else
            {
        ...
        public override int ReadByte()
        {
            ...
            if (this.internalStream is not null)
            {
                r = this.internalStream.ReadByte();
                this.internalStreamProgress += r;
                OnProgress(this.internalStreamProgress, this.Length);
            }
            else

...
namespace DanTheMan827.Modulation
{
    internal static class Readme
    {
        ...
        public static string GenerateReadme(params Song[] songs)
        {
            ...
            foreach (var song in songs.OrderBy(song => song.CleanName).OrderBy(song => song.CleanArtist))
            {
                songListHtml.AddRange(new string[] {
                    ...
                    !string.IsNullOrWhiteSpace(song.MoggSong?.DemoVideo) ? $" - \n{Indent(2)}<a href=\"{HttpUtility.HtmlEncode(song.MoggSong.DemoVideo.Trim())}\" target=\"_blank\">\n{Indent(3)}Demo Video\n{Indent(2)}</a>\n" : "\n",
                    $"{Indent(1)}</div>\n",

                    $"{Indent(1)}<div class=\"id\">\n{Indent(2)}<strong>ID: </strong>\n{Indent(2)}{HttpUtility.HtmlEncode($"{song.SongFolder}")}\n{Indent(1)}</div>\n",
                    (song.MoggSong?.Bpm != null) ? $"{Indent(1)}<div class=\"bpm\">\n{Indent(2)}<strong>BPM: </strong>\n{Indent(2)}{HttpUtility.HtmlEncode($"{song.MoggSong.Bpm}")}\n{Indent(1)}</div>\n" : "",
                    !string.IsNullOrWhiteSpace(song.MoggSong?.Charter) ? $"{Indent(1)}<div class=\"charter\">\n{Indent(2)}<strong>Charter: </strong>\n{Indent(2)}{HttpUtility.HtmlEncode(song.MoggSong.Charter.Trim())}\n{Indent(1)}</div>\n" : "",
                    $"{Indent(1)}<div class=\"content\">",
                    !string.IsNullOrWhiteSpace(song.MoggSong?.CleanDescription) ? $"\n{Indent(2)}<p>{HttpUtility.HtmlEncode(song.MoggSong.CleanDescription.Trim())}</p>\n{Indent(1)}</div>\n" : "</div>\n",
                    $"{Indent(0)}</div>"
                });
            }
